...
import { ProviderError, ProviderAuthError, ProviderRateLimitError, ProviderQuotaError, ProviderInvalidResponseError, TimeoutError, CancellationError, } from "@qwen-claude/core";
...
export class HttpProvider extends BaseProvider {
     ...
     */
    async makeRequest(config, cancellationToken) {
        try {
     ...
     */
    async *makeStreamingRequest(config, cancellationToken) {
        try {
            const controller = this.setupCancellation(cancellationToken);
            const response = await this.client.request({
                ...config,
                ...
                for (const line of lines) {
                    ...
                    if (trimmed.startsWith("data: ")) {
                        ...
                        try {
                            ...
                            yield {
                                success: false,
                                error: new ProviderInvalidResponseError(this.name, data, undefined, parseError instanceof Error
                                    ? parseError
        ...
        if (axios.isAxiosError(error)) {
            ...
            switch (status) {
                ...
                case 429:
                    ...
                    return new ProviderRateLimitError(this.name, retryAfter ? parseInt(retryAfter, 10) : undefined, { status, message });
                case 402:
                case 403:
                    return new ProviderQuotaError(this.name, { status, message });
                case 422:
                    return new ProviderInvalidResponseError(this.name, error.response?.data, { status, message });
                default:
                    if (error.code === "ECONNABORTED") {
                        return new TimeoutError(`Request to ${this.name} timed out`, this.config.timeout || 30000, { status: status ?? null, message });
                    }
                    return new ProviderError(`HTTP error from ${this.name}: ${message}`, this.name, undefined, { status: status ?? null, message });
            }
        }
