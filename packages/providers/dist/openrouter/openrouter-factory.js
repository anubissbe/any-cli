    ...
    validateConfig(config) {
        try {
            ...
            if (parsed.type !== "remote") {
                return {
                    success: false,
                    error: new ValidationError('OpenRouter provider must be of type "remote"'),
                };
            }
            if (parsed.auth.type !== "api_key") {
                return {
                    success: false,
                    error: new ValidationError("OpenRouter provider requires API key authentication"),
                };
            }
            if (!parsed.auth.apiKey) {
                return {
                    success: false,
                    error: new ValidationError("OpenRouter provider requires apiKey in auth configuration"),
                };
            }
            // Set default values
            ...
            return {
                success: false,
                error: new ValidationError("Invalid OpenRouter provider configuration", undefined, error instanceof Error ? error : new Error(String(error))),
            };
        }
    }
}
//# sourceMappingURL=openrouter-factory.js.map