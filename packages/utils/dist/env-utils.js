 ...
 */
export function getEnvInteger(key, defaultValue) {
 ...
 */
export function getEnvArray(key, defaultValue = []) {
    const value = process.env[key];
    if (!value)
        return defaultValue;
    return value
 ...
 */
export function getEnvJSON(key, defaultValue) {
    const value = process.env[key];
    if (!value)
        return defaultValue;
    try {
 ...
 */
export function getEnvWithPrefix(prefix) {
    const result = {};
 ...
 */
export function getEnvConfig(schema) {
    const config = {};
    for (const [configKey, options] of Object.entries(schema)) {
        const { key, required = false, type = "string", defaultValue, validator, } = options;
        let value;
        try {
            switch (type) {
                case "string":
                ...
                case "json":
                    ...
                    break;
                default:
                    throw new Error(`Unknown type ${type} for environment variable ${key}`);
            }
            // Custom validation
 ...
 */
export function createEnvKey(prefix, ...path) {
    return [prefix, ...path]
        .join("_")
        .toUpperCase()
        .replace(/[^A-Z0-9_]/g, "_");
